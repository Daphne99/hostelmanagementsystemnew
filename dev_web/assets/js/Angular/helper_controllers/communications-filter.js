CuteBrains.controller('communicationsFilterController', function(dataFactory, $rootScope, $route, $scope, $location, $routeParams, $http ) {
	  $scope.filter_view = {};
	  $scope.classes = {};
	  $scope.sections = {};
	  $scope.subjects = {};
	  ...
	  $scope.linkStudentFinish = function(student){
		...
		$scope.sendMessageNow = function(){
			...
			dataFactory.httpRequest('index.php/messages','POST',{},$scope.form).then(function(data) {
				...
				if(data.messageId == "home") { $scope.changeView('list'); }
				else { $location.path('/messages/'+data.messageId); }
				showHideLoad(true);
			});
		}

		// for new temp messages section
		$scope.sendTempMessageNow = function(){
			collectSelectedRecipients();
			...
			var err = "Caution";
			if($scope.form.toId.length <= 0) { alertify.alert( err, 'Please select recipients'); return; }
			if($scope.form.toId.length > 1000) { alertify.alert( err, 'Maximum sending to users is 1000, Please reduce the number and resend to other users.'); return; }
			if(typeof $scope.form.messageText == 'undefined') { alertify.alert( err, 'Please write message replay'); return; }
			showHideLoad();
			dataFactory.httpRequest('index.php/messaging/compose','POST',{},$scope.form).then(function(data) {
		...
		// for new events section
		$scope.composeEventNow = function(){
			collectSelectedRecipients();
			alertify.defaults.transition = 'zoom';
			alertify.defaults.theme.ok = 'btn btn-sm btn-info';
			alertify.defaults.theme.cancel = 'btn btn-sm btn-danger';
			...
			if($scope.form.toId.length <= 0) { alertify.alert( err, 'Please select recipients'); return; }
			if($scope.form.toId.length > 1000) { alertify.alert( err, 'Maximum sending to users is 1000, Please reduce the number and resend to other users.'); return; }
			if(typeof $scope.form.eventTitle == 'undefined') { alertify.alert( err, 'Please write event title'); return; }
			if(typeof $scope.form.eventStart == 'undefined') { alertify.alert( err, 'Please write event start date'); return; }
			// if( !$('#eventImage').val() ) { alertify.alert( err, 'Please select image for the event'); return; }
			var model = {
				title: $scope.form.eventTitle,
				description: $scope.form.eventDesc ? $scope.form.eventDesc : '',
				startDate: $scope.form.eventStart ? $scope.form.eventStart : '',
				endDate: $scope.form.eventEnd ? $scope.form.eventEnd : '',
				image: document.getElementById('eventImage').files[0],
				recipients: $scope.form.toId
			}
			var configs = {
			...
			showHideLoad();
			$http.post('index.php/eventous/createNewEvent', model, configs).then(function(data) {
				let xhrResponse = data.data;
				showHideLoad(true);
				if( xhrResponse.status == 'success' )
				{
					response = apiResponse(xhrResponse, 'edit');
					$location.path('/tmp-events');
		...
		// for new notice section
		$scope.sendTempNoticeNow = function(){
			collectSelectedRecipients();
			alertify.defaults.transition = 'zoom';
			alertify.defaults.theme.ok = 'btn btn-sm btn-info';
			alertify.defaults.theme.cancel = 'btn btn-sm btn-danger';
			...
			if($scope.form.toId.length <= 0) { alertify.alert( err, 'Please select recipients'); return; }
			if($scope.form.toId.length > 1000) { alertify.alert( err, 'Maximum sending to users is 1000, Please reduce the number and resend to other users.'); return; }
			if(typeof $scope.form.noticeTitle == 'undefined') { alertify.alert( err, 'Please write notice title'); return; }
			if(typeof $scope.form.noticeDate == 'undefined') { alertify.alert( err, 'Please write notice date'); return; }
			if(typeof $scope.form.noticeContent == 'undefined') { alertify.alert( err, 'Please write notice content'); return; }
			// if( !$('#noticeImage').val() ) { alertify.alert( err, 'Please select image for the notice'); return; }
			var model = {
				title: $scope.form.noticeTitle,
				description: $scope.form.noticeContent ? $scope.form.noticeContent : '',
				date: $scope.form.noticeDate ? $scope.form.noticeDate : '',
			...
			showHideLoad();
			$http.post('index.php/notices/createNewNotice', model, configs).then(
				function(data) {
					let xhrResponse = data.data;
					showHideLoad(true);
					if( xhrResponse.status == 'success' )
		...
		// for new mail/sms section
		$scope.sendTempMailSmsNow = function(){
			collectSelectedRecipients();
			alertify.defaults.transition = 'zoom';
			alertify.defaults.theme.ok = 'btn btn-sm btn-info';
			alertify.defaults.theme.cancel = 'btn btn-sm btn-danger';
			alertify.defaults.theme.input = 'form-control';
			var err = "Caution";if($scope.form.toId.length <= 0) { alertify.alert( err, 'Please select recipients'); return; }
			if($scope.form.toId.length > 1000) { alertify.alert( err, 'Maximum sending to users is 1000, Please reduce the number and resend to other users.'); return; }
			showHideLoad();
			dataFactory.httpRequest('index.php/mailandsms/createNewMailSms', 'POST', {}, $scope.form ).then(function(data) {
				showHideLoad(true);
				if( data.status == "success" )
				{
					response = apiResponse(data, 'edit');
					$location.path('/tmp-mailsms');
				}
				else if( data.status == "failed" ) { response = apiResponse(errorMsg, 'remove'); }
				else { response = apiResponse(errorMsg, 'remove'); }
			});
		}
