var CuteBrains = angular.module('homeworkController', []);

CuteBrains.controller('homeworkController', function(dataFactory,$rootScope,$scope,$sce) {
    $scope.classes = {};
    $scope.filterClasses = {};
    ...
    $scope.preLoad = function(){
        showHideLoad();
        dataFactory.httpRequest('homeworks/preload').then(function(data) {
            $scope.filterClasses = data.classes;
            showHideLoad(true);
        });
    }

    $scope.loadSections = function(){
        var classId = $scope.filterForm.class;
        if( classId != "" ) { $scope.filterSections = $scope.filterClasses[classId].sections; }
        else { $scope.filterSections = {}; }
    }
    ...
    $scope.filterDB = function()
    {
        var pageNumber = 1;
        showHideLoad();
        var send_data = {class: $scope.filterForm.class, section: $scope.filterForm.section};
        dataFactory.httpRequest('index.php/homeworks/listAll/' + pageNumber, 'GET', send_data).then(function(data) {
    ...
    $scope.load_data = function(pageNumber) {
        ...
        if(! $.isEmptyObject($scope.homeworksTemp)){
            ...
            showHideLoad();
            var send_data = {class: $scope.filterForm.class, section: $scope.filterForm.section};
            dataFactory.httpRequest('index.php/homeworks/search/'+$scope.searchText+'/' + pageNumber, 'GET', send_data).then(function(data) {
                $scope.classes = data.classes;
                $scope.subject = data.subject;
                $scope.homeworks = data.homeworks;
                $scope.userRole = data.userRole;
