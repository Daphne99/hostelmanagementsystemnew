var CuteBrains = angular.module('institutionTimetableTeacherwiseController', []);

CuteBrains.controller('institutionTimetableTeacherwiseController', function(dataFactory,$rootScope,$scope,$sce) {
		...
		$scope.currentDisplayDay = getCurrentDay();
		$scope.search = {};
		$scope.search.teacher_name = '';
		$scope.searchStatus = false;
		$scope.importDataForm = {};
		$scope.importDataForm.warnings = '0';
    ...
    $scope.saveImported = function(content){
      ...
      if(content){
        ...
        } else if(content.status == 'asking') {
        	if(confirm(content.title)) {
        		$scope.importDataForm.warnings = '1';
        		setTimeout(function(){
        			$('#importTimetableForm').submit();
        		}, 500)
        	} else {
      			showHideLoad(true);
        	}
