var CuteBrains = angular.module('teacherAvailabilityController', []);

CuteBrains.controller('teacherAvailabilityController', function(dataFactory,$rootScope,$scope,$sce) {
  ...
  $scope.searchDB = function(pageNumber){
   	showHideLoad();
   	console.log(JSON.stringify($scope.searchInput.subject));
    dataFactory.httpRequest('index.php/classschedule/teacher-availability/' + pageNumber, 'GET', {
    	...
    	'filter_section': JSON.stringify($scope.searchInput.section),
    	'filter_subject': JSON.stringify($scope.searchInput.subject)
    }).then(function(data) {
        $scope.availabilities = data.availabilities;
        angular.forEach($scope.availabilities, function (item) {
            item.teacher_info = $sce.trustAsHtml(item.teacher_info);
