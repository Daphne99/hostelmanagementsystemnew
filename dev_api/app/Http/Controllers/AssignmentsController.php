...
class AssignmentsController extends Controller {
	...
	public function listAll()
	{
		...
		if(count($classesArray) > 0)
		{
			...
			if($this->data['users']->role == "teacher")
			{
				$assignments = $assignments->where('teacherId', $this->data['users']->id)->where(
					function($query) use ($classesArray) {
						foreach($classesArray as $key => $value) { $query = $query->orWhere('classId','LIKE','%"'.$key.'"%'); }
					}
				);
			...
			elseif($this->data['users']->role == "student")
			{
				...
				if($this->panelInit->settingsArray['enableSections'] == true) { $assignments = $assignments->where('sectionId','LIKE','%"'.$this->data['users']->studentSection.'"%'); }
				$assignments = $assignments->orderBy('id', 'DESC');
				$assignments = $assignments->get();
			}
			elseif( $this->data['users']->role == "parent" )
			{
				...
				if( !is_array( $parentOf ) ) { $parentOf = array(); }
				$ids = array();
				foreach( $parentOf as $value ) { $ids[] = $value['id']; }
				if( count( $ids ) == 0 ) { unset($assignments); }
				...
				if(count($ids) > 0)
				{
					...
					foreach( $studentArray as $stOne )
					{
						...
						if( $this->panelInit->settingsArray['enableSections'] == true ) { $sectionArray[] = $stOne->studentSection; }
					}

					if(count($classArray) > 0)
					{
						$assignments = $assignments->where(
							function($query) use ($classArray) {
								foreach($classArray as $value) { $query = $query->orWhere('classId','LIKE','%"'.$value.'"%'); }
							}
						);
					}
					...
					{
						$assignments = $assignments->where(
							function($query) use ($sectionArray) {
								foreach($sectionArray as $value) { $query = $query->orWhere('sectionId','LIKE','%"'.$value.'"%'); }
							}
						);
					}
					$assignments = $assignments->orderBy('id', 'DESC');
			...
			foreach ($assignments as $key => $assignment) {
				...
				if(\File::exists($images_dir) && !empty($item_dir)) {
					...
					{
						$_image_path = str_replace( uploads_config()['uploads_file_path'] . "/assignments/$item_dir/", "", $image_path );
						// $_image_path = str_replace( uploads_config()['uploads_file_path'], uploads_config()['uploads_provider'], $image_path );
						$dir_images_array[] = $_image_path;
					}
					$dir_images_array = $this->sort_pdf_image_array($dir_images_array);
					$toReturn['assignments'][$key]['dir'] = $item_dir;
	...
	public function download($id){
		if(!$this->panelInit->can( array("Assignments.Download") )) { exit; }
		$toReturn = \assignments::where('id',$id)->first();
		if( trim( $toReturn->AssignFile ) == "" )
		{
			echo "<br/><br/><br/><br/><br/><center>File not exist, Please contact site administrator to reupload it again.</center>";
			exit;
		}
		if( file_exists(uploads_config()['uploads_file_path'] . '/assignments/'.$toReturn->AssignFile) == true )
		{
	...
	public function create(){
		...
		if (\Input::hasFile('AssignFile')) {
			...
			$allowable_mimeTypes = [
				"image/png", "image/jpeg", "text/pdf", "image/pjpeg", "application/pdf","application/x-pdf", "application/msword",
				"application/vnd.openxmlformats-officedocument.wordprocessingml.document",
				"application/excel", "application/vnd.ms-excel", "application/x-msexcel","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
			];

			if( !in_array($fileInstance->getMimeType(), $allowable_mimeTypes) ) { return $this->panelInit->apiOutput(false,$this->panelInit->language['AddAssignments'],"Sorry, This File Type Is Not Permitted For Security Reasons "); }

			$newFileNameWithoutExt = "assignments_".uniqid();
			$newFileName = $newFileNameWithoutExt.".".$fileInstance->getClientOriginalExtension();
			$newFileExt = $fileInstance->getClientOriginalExtension();
		...
		dispatch(new SendAssignmentNotification(
			\Input::get('classId'),
			\Input::get('sectionId'),
			$this->panelInit->settingsArray['enableSections'],
			$this->panelInit->language['newAssigmentAdded'],
	...
	function edit($id){
		...
		if (\Input::hasFile('AssignFile')) {
			...
			if( !in_array($fileInstance->getMimeType(), $allowable_mimeTypes) ) { return $this->panelInit->apiOutput(false,$this->panelInit->language['editAssignment'],"Sorry, This File Type Is Not Permitted For Security Reasons "); }
	...
	public function readImage( $dir , $image ){
		header('Content-Type: image/jpeg');
		$uploads_file_path = uploads_config()['uploads_file_path'];
		if( file_exists( $uploads_file_path . "/assignments/$dir/$image" ) ) { return file_get_contents($uploads_file_path . "/assignments/$dir/$image"); }
		else { return ""; }
	}
}