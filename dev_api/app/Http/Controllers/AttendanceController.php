...
use Carbon\Carbon;
use App\Models2\HR\ViewEmployeeInOutData;
...
class AttendanceController extends Controller {
	...
	public function getStats($date = ""){
		...
		if($this->data['users']->role == "teacher")
		{
			...
			$classes = \classes::where('classAcademicYear',$this->panelInit->selectAcYear)
				...
				->get();
		} else { $classes = \classes::where('classAcademicYear',$this->panelInit->selectAcYear)->get(); }
	...
	protected function sectionsList($classId = ""){
		...
		if(Auth::guard('web')->user()->role == "teacher") { $query->where('classTeacherId', 'LIKE',  '%"' . $teacher_id . '"%'); }
	...
	public function getAbsentStudents() {
		...
		$now = Carbon::now();
		$startDate = $now->startOfDay()->timestamp;
		$endDate = $now->endOfDay()->timestamp;
		$target_student_ids = Attendance::whereBetween('date', [$startDate, $endDate])
			->where('status', 0)
			->groupBy('studentId')
			->pluck('studentId')
			->toArray();
		...
		foreach ($target_student_ids as $id => $student_id) {
			if(isset($needed_users[$student_id])) {
				$section_id = $needed_users[$student_id];

				if(!isset($absent_sections[$section_id])) {
					$absent_sections[$section_id] = 1;
				} else {
					$absent_sections[$section_id] += 1;
				}
			}
		...
		foreach ($classes as $class_id => $class_name) {
			...
			foreach ($sections as $section_id => $section_name) {
				if(in_array($section_id, array_keys($absent_sections))) {
					$item["Section " . $section_name] = $absent_sections[$section_id];
				}
			}
	...
	public function getAbsentStaff() {
		$sub_0 = Carbon::now()->startOfDay()->format('y/m/d');
		$sub_1 = Carbon::now()->subDays(1)->format('y/m/d');
		$sub_2 = Carbon::now()->subDays(2)->format('y/m/d');
		$sub_3 = Carbon::now()->subDays(3)->format('y/m/d');
		$sub_4 = Carbon::now()->subDays(4)->format('y/m/d');
		$sub_5 = Carbon::now()->subDays(5)->format('y/m/d');
		$sub_6 = Carbon::now()->subDays(6)->format('y/m/d');
		$sub_7 = Carbon::now()->subDays(7)->format('y/m/d');
		$sub_8 = Carbon::now()->subDays(8)->format('y/m/d');
		$sub_9 = Carbon::now()->subDays(9)->format('y/m/d');
		...
		$dimensions = array_merge(['date'], $days);

		$items = [];
		$items2 = [];
		$counter = 0;
		foreach ($days as $key => $sub) {
			if($counter % 2 != 0) {
				$sub = "\n" . $sub;
			}

			$item = ['date' => $sub];
			$item[$sub] = ViewEmployeeInOutData::whereBetween('date', [
				Carbon::now()->subDays($key + 1),
				Carbon::now()->subDays($key),
			])->groupBy('employee_attendance_id')->pluck('employee_attendance_id')->toArray();

			$items2[] = count($item[$sub]);
