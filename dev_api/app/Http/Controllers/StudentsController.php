...
class StudentsController extends Controller {
	...
	public function listAll($page = 1)
	{
		...
		if(\Input::has('searchInput') AND isset($students)){
			...
			if(is_array($searchInput)){
				...
				if(isset($searchInput['vehicle']) AND $searchInput['vehicle'] != "" ){
					$students = $students->where('transport_vehicle', $searchInput['vehicle']);
				}
		...
		foreach ($transport_vehicles_ as $key => $value) {
			...
			$vehicles[] = [ "id" => $value['id'], "name" => $value['plate_number'] . " ( " .$value['driver_name'] ." )" ];
		}

		$toReturn['transports'] = array();
		$toReturn['transport_vehicles'] = $vehicles;
	...
	public function create(){
		...
		if(!is_null(\Input::has('std_type')) && \Input::get('std_type') > 0){
		...
		if(!is_null(\Input::has('generate_fee_collections')) && \Input::has('generate_fee_collections') == true){
			$student = [];
			if(!is_null(\Input::has('std_type')) && \Input::get('std_type') > 0){
				$student['type_id'] = \Input::get('std_type');
			} else {
				$student['type_id'] = 0;
			}
			$student['class_id'] = \Input::get('studentClass');
			$student['section_id'] = \Input::get('studentSection');

			// Get payment students ----------------------------------------------
				User::$withoutAppends = true;
				$students = User::where('role','student')->where('activated','1');
				if($student['class_id'] > 0){
					$students = $students->where('studentClass', $student['class_id']);
				}
				if($student['section_id'] > 0){
					$students = $students->where('studentSection', $student['section_id']);
				}
				if($student['type_id'] > 0){
					$students = $students->where('studentType', $student['type_id']);
				}
			...
			if(count($paymentStudent)) {
				...
				if(count($bulk_fees)) {
					foreach ($bulk_fees as $fee) {
						...
						$payment->paymentStudent = $User->id;
	...
	function profile($id){
		...
		if($data->count() > 0){
			...
			$return['modalClass'] = "modal-lg";

			$className = isset($class) ? $class->className : "";
			$sectionName = isset($section) ? $section->sectionName : "";
					...
					$content .= "<div class='card'>";
						$content .= "<div class='card-block'>";
							$content .= "<div class='card-body'>";
								$content .= "<div class='row'>";
									$content .= "<div class='col-4 pr-1 pl-1'>";
										$content .= "<div class='card'>";
											$content .= "<div class='card-block' style='background-color: white !important;'>";
												$content .= "<div class='card-body'>";
													$content .= "<div class='row'>";
														$content .= "<div class='col-12 text-center'>";
															$content .= "<img alt='".$data['fullName']."' style='border: 3px solid #ccc !important; border-radius: 50% !important; width: 96px !important; height: 96px !important; padding: 5px !important;' src='index.php/dashboard/profileImage/".$data['id']."'> <br>";
															$content .= "<h4><b>" . $data['fullName'] . "</b></h4>";
														$content .= "</div>";
														$content .= "<div class='col-12 text-center'> <small style='color: #aaa !important;'>Student</small> </div>";
													$content .= "</div>";
													$content .= "<div class='row'>";
														$content .= "<div class='col-12'>";
															$content .= "<table class='table table-hover' style='border: 1px solid #eceeef !important;'>";
																$content .= "<tbody><tr> <td style='font-size: small;'>Admission No.</td> <td style='font-size: small;' class='text-right'> " . $data['admission_number'] . " </td> </tr>";
																$content .= "<tr> <td style='font-size: small;'>Roll</td> <td style='font-size: small;' class='text-right'> " . $data['studentRollId'] . " </td> </tr>";
																$content .= "<tr> <td style='font-size: small;'>Class</td> <td style='font-size: small;' class='text-right'> " . $className . " </td> </tr>";
																$content .= "<tr> <td style='font-size: small;'>Section</td> <td style='font-size: small;' class='text-right'> " . $sectionName . " </td> </tr>";
															$content .= "</tbody></table>";
														$content .= "</div>";
													$content .= "</div>";
												$content .= "</div>";
										...
										$content .= "</div>";
									$content .= "</div>";
									$content .= "<div class='col-8 pr-1 pl-1'>";
										$content .= "<div class='card'>";
											$content .= "<div class='card-block' style='background-color: white !important;'>";
												$content .= "<div class='card-body'>";
													$content .= "<div class='container-fluid'>";
														$content .= "<div class='row mt-2 mb-2'>";
								  							$content .= "<div class='col-3' style='font-size: small;'> Username </div>";
								  							$content .= "<div class='col-9' style='font-size: small;'> " . $data['username'] . " </div>";
														$content .= "</div>";
														$content .= "<div class='row mt-2 mb-2'>";
															$content .= "<div class='col-3' style='font-size: small;'> Mail </div>";
								  							$content .= "<div class='col-9' style='font-size: small;'> " . $data['email'] . " </div>";
														$content .= "</div>";
														$content .= "<div class='row mt-2 mb-2'>";
															$content .= "<div class='col-3' style='font-size: small;'> Birthday </div>";
															$content .= "<div class='col-9' style='font-size: small;'> " . $data['birthday'] . " </div>";
														$content .= "</div>";
														$content .= "<div class='row mt-2 mb-2'>";
															$content .= "<div class='col-3' style='font-size: small;'> Gender </div>";
															$content .= "<div class='col-9' style='font-size: small;'> " . $data['gender'] . " </div>";
														$content .= "</div>";
														$content .= "<div class='row mt-2 mb-2'>";
															$content .= "<div class='col-3' style='font-size: small;'> Mobile No </div>";
															$content .= "<div class='col-9' style='font-size: small;'> " . $data['mobileNo'] . " </div>";
														$content .= "</div>";
														$content .= "<div class='row mt-2 mb-2'>";
															$content .= "<div class='col-3' style='font-size: small;'> Parent </div>";
															$content .= "<div class='col-9' style='font-size: small;'>";
																foreach ($parents as $value) { $content .= $value->fullName . "<br/>"; }
															$content .= "</div>";
														$content .= "</div>";
														$content .= "<div class='row mt-2 mb-2'>";
															$content .= "<div class='col-3' style='font-size: small;'> Phone No </div>";
															$content .= "<div class='col-9' style='font-size: small;'> " . $data['phoneNo'] . " </div>";
														$content .= "</div>";
														$content .= "<div class='row mt-2 mb-2'>";
															$content .= "<div class='col-3' style='font-size: small;'> Address </div>";
															$content .= "<div class='col-9' style='font-size: small;'> " . $data['address'] . " </div>";
														$content .= "</div>";
													$content .= "</div>";
												$content .= "</div>";
											$content .= "</div>";
