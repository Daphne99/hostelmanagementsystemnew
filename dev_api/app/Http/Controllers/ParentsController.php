	...
	function listAllData($page = 1){
		...
		if(\Input::has('searchInput')){
			...
			if(is_array($searchInput)){
				...
				if(isset($searchInput['class']) AND count($searchInput['class']) > 0 && isset($searchInput['sections']) AND count($searchInput['sections']) > 0){
					$parent_ids = [];
					$section_ids = $searchInput['sections'];
					$student_ids = User::getStudentsIdsFromSectionsIds($section_ids);
					foreach ($student_ids as $student_id) {
						$parent_id = User::getRealParentIdsFromStudentId($student_id);
						if(!is_null($parent_id) && !empty($parent_id) && !in_array($parent_id, $parent_ids)) {
							$parent_ids[] = $parent_id;
						}
					}
					$toReturn['parents'] = $toReturn['parents']->whereIn('id', $parent_ids);
				}
