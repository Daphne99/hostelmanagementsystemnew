...
class homeworksController extends Controller {
	...
	public function preload()
	{
        ...
        $sections = Section::select('id', 'sectionName as name', 'classId')->get()->toArray();
	...
	public function listAll($page = 1,$search = "")
	{
		...
		} else { $toReturn['classes'] = \classes::where('classAcademicYear',$this->panelInit->selectAcYear)->get()->toArray(); }
		...
		if(count($classesArray) > 0){
			...
			if( \Input::has('class') )
			{
				$classId = trim( \Input::get('class') );
				if( $classId ) { $homeworks = $homeworks->where("classId","like",'%"'.$classId.'"%'); }
			}

			if( \Input::has('section') )
			{
				$sectionId = trim( \Input::get('section') );
				if( $sectionId ) { $homeworks = $homeworks->where("sectionId","like",'%"'.$sectionId.'"%'); }
			}
