...
class MessagesController extends Controller {
	...
	public function peek( $messageId )
	{
		$user_id = $this->data['users']->id;
		$message = \messages::where('messageId', $messageId)->where('toId', $user_id)->orderBy('id', 'DESC')->first();
		if( !$message ) return $this->panelInit->apiOutput(false, "peek message", "No new messages");
		else
		{
			User::$withoutAppends = true;
			$messageDet = $message->toArray();
			$messageDet['dateSentH'] = $this->panelInit->unix_to_date($messageDet['dateSent'], $this->panelInit->settingsArray['dateformat']." hr:mn a");
			$sender = User::select('id', 'fullName as name')->where('id', $messageDet['fromId'])->first();
			$messageDet['sender'] = $sender ? $sender->name : "Unknown";
			$outPut = [ "status" =>	"success", "title" => "peek message", "message" => "Data Found", "details" => $messageDet ];
			return $outPut;
		}
	}
	...
	public function create(){
		...
		$is_enable_reply = !is_null( \Input::get('is_enable_reply') ) ? 1 : 0;
		$currentUser = $this->data['users']->id;
		$fullName = $this->data['users']->fullName;
		$settings = (array) $this->panelInit->settingsArray;
		dispatch( new SendBulkMessages( $toId, $messageText, $is_enable_reply, $currentUser, $fullName, $settings ) );
