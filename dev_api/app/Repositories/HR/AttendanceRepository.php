...
use App\libraries\HR\Enumerations\LeaveStatus;

use App\libraries\HR\Enumerations\UserStatus;
...
class AttendanceRepository
{
    ...
    public function findAttendanceSummaryReport($month){
        ...
        $employees  = Employee::select(DB::raw('CONCAT(COALESCE(hr_employee.first_name,\'\'),\' \',COALESCE(hr_employee.last_name,\'\')) AS fullName'),'designation_name','finger_id','employee_id')
                    ->join('hr_designation','hr_designation.designation_id','hr_employee.designation_id')
                    ->where('status',UserStatus::$ACTIVE)->get();

        $start_date = $month.'-01';
        $end_date   = date("Y-m-t", strtotime($start_date));

        $attendance = DB::table('hr_view_employee_in_out_data')->select('finger_print_id','date')->whereBetween('date',[$start_date,$end_date])->get();
        $leave      = LeaveApplication::select('application_from_date','application_to_date','employee_id','leave_type_name')
                    ->join('hr_leave_type','hr_leave_type.leave_type_id','hr_leave_application.leave_type_id')
                    ->where('status',LeaveStatus::$APPROVE)
