...
class SendAttendance implements ShouldQueue
{
    ...
    public function __construct($stAttendance, $class_id, $attendanceDay, $subjectId, $attendanceModel, $absentNotif, $attendanceDay_input, $settingsArray, $currentDate)
    {
				...
				if(isset($value['attendance']) AND strlen($value['attendance']) > 0){
					...
					if($this->absentNotif != "0"){
						...
						foreach ($parents as $parent) {
							...
							echo '['. $this->currentDate .'] Save notification to DB for: ' . $student->fullName . PHP_EOL;
						}
					}
				}
			}
