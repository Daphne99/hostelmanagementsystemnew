            ...
            <?php
              foreach ($panelInit->menuElement as $key => $value) {
                ...
                if(is_array($value)) {
                  ...
                  if( array_key_exists('icon', $value) ) { echo "<i ". $style ." class='".$value['icon']."'></i><span class='hide-menu'>"; }
                  if( array_key_exists('title', $value) )
                  {
                      if(isset($panelInit->language[$value['title']])){
                        echo $panelInit->language[$value['title']];
                    }else{
                      if( array_key_exists('title', $value) ) { echo $value['title']; }
                    }
                  }
                  if( array_key_exists('icon', $value) ) { echo "</span>"; }
                  echo "</a>";
                  if(isset($value['children'])){
