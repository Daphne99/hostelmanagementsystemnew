...
<li class="nav-item dropdown" style="width:45px;">
  <a class="nav-link dropdown-toggle notification-header text-muted waves-effect waves-dark" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="margin-top: 8px;">
...
@section('scripts')
	<script type="text/javascript">
		$(document).ready(function(){
			// var notif_interval_counter = 3000;
			var notif_interval_counter = 4 * 60 * 1000;
      ...
      function getNotificationCounterAndData() {
      	$.ajax({
					url: 'index.php/get-notification-count-alert',
					type: 'get',
					success: function(res){
						// set notification counter alert
						if($('#notification-count-alert').length != 0) {
							$('#notification-count-alert').text(res.unseen_notif_count);
						} else {
							$('.dropdown .notification-header').prepend(`
								<div class="bg-danger notification-count-alert" id="notification-count-alert">
					    		${res.unseen_notif_count}
					    	</div>
							`);
						}
      ...
      function resetNotificationSeenStatus() {
      	$.ajax({
					url: 'index.php/reset-notifications-seen-status',
					type: 'get',
					success: function(res){
			...
			setInterval(function(){
				getNotificationCounterAndData();
			}, notif_interval_counter);

			$('.dropdown').delegate('.dropdown-toggle', 'click', function(){
				resetNotificationSeenStatus();
			});

			$('#notification-dropdown').delegate('.item', 'click', function(){
