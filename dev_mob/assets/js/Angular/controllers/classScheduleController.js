var CuteBrains = angular.module('classScheduleController', []);

CuteBrains.controller('classScheduleController', function(dataFactory,$rootScope,$scope,$sce) {
    $scope.classes = {};
    $scope.subject = {};
    $scope.days = {};
    $scope.classSchedule = {};
    ...
    $scope.current_user_role = $rootScope.dashboardData.role;
    ...
    dataFactory.httpRequest('index.php/classschedule/listAll').then(function(data) {
        ...
        if($scope.sections_collection.length) {
    ...
    $scope.fetchClassschedule1 = function(class_id,section_id){
        ...
        dataFactory.httpRequest('index.php/classschedule/'+$scope.query_id).then(function(data) {
            ...
            $scope.setDayName(getCurrentDay());
            showHideLoad(true);
        });
    }

    $scope.fetchClassschedule2 = function(class_id,section_id){
        showHideLoad();
        dataFactory.httpRequest('index.php/classschedule-teacher').then(function(data) {
            $scope.classSchedule = data;
            $scope.setDayName(getCurrentDay());
            showHideLoad(true);
        });
    ...
    if($rootScope.dashboardData.role == 'teacher') {
    	$scope.fetchClassschedule2();
    }
