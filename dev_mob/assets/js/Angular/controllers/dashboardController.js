...
CuteBrains.controller('dashboardController', function(dataFactory,$rootScope,$scope,$sce) {
    ...
    $scope.server_info = JSON.parse($rootScope.dashboardData.server_info);
		...
		$scope.modalTitle = 'Important Notice Regrading Offline Fee Payment';
    $scope.modalTitleClass = "text-warning";

		$scope.dontShowPopupSessionStatus = sessionStorage.getItem("dontShowPopupSessionStatus");
		$scope.gpsModalStatus = false;
		$scope.gpsModal2Status = false;
    ...
    $scope.filterStudentsByClassAndSection = function(pageNumber) {
    		...
    		if(($scope.filter_students_input_class != null && $scope.filter_students_input_class != '0') || ($scope.filter_students_input_section != null && $scope.filter_students_input_section != '0')) {
    			send_data = {
	    			searchInput: {
	    				'class': input_class,
    ...
    $scope.getStudentsDataWithPaginate = function(pageNumber) {
    	if($scope.filter_students_input_class > 0 || $scope.filter_students_input_section > 0) {
    		// get student in (class/section) filter case paginate
    		$scope.filterStudentsByClassAndSection(pageNumber);
    	} else {
	    	showHideLoad();
    ...
    $scope.getStudentsResultsPage = function(newpage = ""){
        ...
        showHideLoad();
        dataFactory.httpRequest('index.php/students/listAll/'+$scope.current_page).then(function(data) {
    ...
    /* ----- trackBus ------ */
	    ...
	    $scope.trackBus = function(e, student_id) {
	      ...
	      dataFactory.httpRequest('index.php/students/get-bus-track-info/'+student_id).then(function(data) {
	          ...
	          $scope.gpsModalStatus = true;
	      		$scope.modalTitle = 'GPS notification';
	      		$scope.modalTitleClass = '';
	      });
	    }
    ...
    $scope.dontShowPopupAgain = function() {
    	sessionStorage.setItem("dontShowPopupSessionStatus", true);
    	$scope.dontShowPopupSessionStatus = true;
    }

    $scope.openGpsInfo2Modal = function() {
    	$scope.gpsModal2Status = true;
    	$scope.modalTitle = 'GPS Alert';
    	$scope.modalTitleClass = '';
    }
});

CuteBrains.filter('capitalize', function() {
