var CuteBrains = angular.module('mediaController', []);

CuteBrains.controller('mediaController', function($rootScope,dataFactory,$scope,Upload,$sce,$timeout) {
    $scope.albums = {};
    $scope.media = {};
    $scope.dirParent = -1;
    $scope.dirNow = 0;
    ...
    $scope.preparGalleryView = function() {
		...
		$scope.getIframeSrc = function (mediaURL) {
		  return $sce.trustAsResourceUrl($scope.uploads_provider + '/media/' + mediaURL);
		};
    ...
    $scope.getUploadsConfig = function() {
    	...
    	dataFactory.httpRequest('index.php/uploads-config', 'POST').then(function(data) {
          $scope.uploads_provider = $sce.trustAsUrl(data.uploads_provider);
          showHideLoad(true);
      });
    }
