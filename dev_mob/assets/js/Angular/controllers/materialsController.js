var CuteBrains = angular.module('materialsController', []);

CuteBrains.controller('materialsController', function(dataFactory,$rootScope,$scope) {
    $scope.classes = {};
    $scope.subject = {};
    $scope.materials = {};
    $scope.views = {};
    ...
    $scope.zoomIn = function(){
      $(".image-item").width(
      	$(".image-item").width() * 1.1
     	)
  		$(".image-item").height(
      	$(".image-item").height() * 1.1
  		);
    }
    $scope.zoomOut = function(){
    	if($(".image-item").width() > $(window).width()) {
	      $(".image-item").width(
	      	$(".image-item").width() * 0.9
	     	)
	  		$(".image-item").height(
	      	$(".image-item").height() * 0.9
	  		);
    	}
    }
