var CuteBrains = angular.module('attendanceController', []);

CuteBrains.controller('attendanceController', function(dataFactory,$scope) {
    $scope.classes = {};
    ...
    dataFactory.httpRequest('index.php/attendance/data').then(function(data) {
        ...
        if($scope.classes.length) {
            if( data.generic )
            {
                $scope.generalData = data.generic;
                if( $scope.generalData.class != 0 && $scope.generalData.section != 0 )
                {
                    $scope.form.classId = $scope.generalData.class;
                    dataFactory.httpRequest('index.php/attendance/get-current-sections-and-subjects','POST',{},{
                        ...
                        if($scope.sections.length) { $scope.form.sectionId = $scope.generalData.section; }
                    });
                } else { $scope.form.classId = $scope.classes[0].id; }
            } else { $scope.form.classId = $scope.classes[0].id; }
