var CuteBrains = angular.module('TransportsController', []);

CuteBrains.controller('TransportsController', function(dataFactory,$scope,$rootScope,$sce) {
    $scope.transports = {};
    ...
    $scope.gpsModalStatus0 = false;
    $scope.gpsModalStatus1 = false;
    // $scope.gpsModalStatusCustomBus7 = false;

    $scope.$on('$viewContentLoaded', function() {
    	document.title = $('meta[name="site_title"]').attr('content') + ' | Stoppages';
    	$scope.modalTitle = 'GPS Buses';
    	$scope.gpsModalStatus0 = true;
    });
    ...
    $scope.load_data = function(){
      ...
      //       var parentHaveBusNo7 = false;
      //       angular.forEach($scope.transports, function(value, key) {
						//   if(value.is_bus_no7 == true) {
						//   	parentHaveBusNo7 = true;
						//   }
						// });

            // if($scope.userRole == 'parent') {
            // 	if(parentHaveBusNo7 == true) {
            // 		$scope.transportsHaveBusNo7 = true;
            // 		$scope.modalTitle = 'GPS - Bus No. 7';
            // 		$scope.gpsModalStatusCustomBus7 = true;
            // 	} else {
            // 		$scope.gpsModalStatusCustomBus7 = false;
            // 	}
            // }
