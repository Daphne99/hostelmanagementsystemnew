var CuteBrains = angular.module('classesController', []);

CuteBrains.controller('classesController', function(dataFactory,$rootScope,$scope) {
    $scope.classes = {};
    $scope.teachers = {};
    // $scope.dormitory = {};
    $scope.subject = {};
    ...
    $scope.filter_classes = {};
    $scope.filter_classes.input_class = '0';
    $scope.filter_classes.input_subject = '0';
    ...
    dataFactory.httpRequest('index.php/classes/listAll').then(function(data) {
        ...
        $scope.subjects = data.subject_array;
    ...
    $scope.filterClassesByClassAndSubject = function() {
    		showHideLoad();
    		var input_class = $scope.filter_classes.input_class;
    		var input_subject = $scope.filter_classes.input_subject;

    		if($scope.filter_classes.input_class != null && $scope.filter_classes.input_class != '0') {
    			input_class = [$scope.filter_classes.input_class];
    		} else {
    			input_class = '';
    		}
    		if($scope.filter_classes.input_subject != null && $scope.filter_classes.input_subject != '0') {
    			input_subject = [$scope.filter_classes.input_subject];
    		} else {
    			input_subject = '';
    		}
    		if(
    			($scope.filter_classes.input_class != null && $scope.filter_classes.input_class != '0')
    			||
    			($scope.filter_classes.input_subject != null && $scope.filter_classes.input_subject != '0')
    		) {
    			send_data = {
	    			searchInput: {
	    				'class': input_class,
    ...
    $scope.getCurrentSubjects = function(){
      $scope.mainSubjectIds = $scope.allClasses.find(item => item.id === parseInt($scope.filter_classes.input_class)).subjects;
    }
